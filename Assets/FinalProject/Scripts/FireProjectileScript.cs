...
namespace DigitalRuby.PyroParticles
{
    ...
    public class FireProjectileScript : FireBaseScript, ICollisionHandler
    {
        ...
        {
			if (c.collider.tag == "monster" && gameObject.tag == "Fire1") {
				monsterHealth = c.gameObject.GetComponent<MonsterHealth> (); 
				monsterHealth.TakeDamage (30);
			}else if (c.collider.tag == "boss" && gameObject.tag == "Fire1") {
				bossHealth = c.gameObject.GetComponent<BossHealth> (); 
				bossHealth.TakeDamage (30);
			}

			if (c.collider.tag == "monster" && gameObject.tag == "Fire2") {
				monsterHealth = c.gameObject.GetComponent<MonsterHealth> (); 
				monsterHealth.TakeDamage (100);
			} else if (c.collider.tag == "boss" && gameObject.tag == "Fire2") {
				bossHealth = c.gameObject.GetComponent<BossHealth> (); 
				bossHealth.TakeDamage (100);
			}

			if (c.collider.tag == "Player" && gameObject.tag == "Fire3") {
